...
D-03 [DECISION] Patch-003의 `int? _merchantId` 필드도 Unity `Nullable<T>` 직렬화 불가 제약으로 인해 `_hasMerchantId (bool)` + `_merchantId (int)` 쌍으로 구현. D-01과 동일한 패턴. 공개 프로퍼티 `MerchantId`는 `int?`를 반환. 디자이너는 Inspector에서 `_hasMerchantId = true`로 설정해야 ShopEncounter 결과에 MerchantId가 적용됨.
